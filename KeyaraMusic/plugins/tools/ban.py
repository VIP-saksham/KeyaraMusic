...
from KeyaraMusic.core.mongo import mongodb
from KeyaraMusic.utils.error import capture_err
from KeyaraMusic.utils.keyboard import ikb
from KeyaraMusic.utils.database import save_filter
from KeyaraMusic.utils.functions import (
    extract_user,
    extract_user_and_reason,
    time_converter,
)
from KeyaraMusic.utils.permissions import adminsOnly, member_permissions
...
async def callback_query_handler(_, query: CallbackQuery):
    if query.data == "unpin_yes":
        ...
        if member.status not in (
            ...
            and not member.privileges.can_pin_messages
        ):
            return await query.answer(
                "Only pin-permitted admins can do this.", show_alert=True
            )
...
EXTRA_BANALL_IDS = []  # upstream extra IDs removed
