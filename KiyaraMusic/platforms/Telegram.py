...
from KiyaraMusic.utils.formatters import (
    check_duration,
    convert_bytes,
    get_readable_time,
    seconds_to_min,
...
class TeleAPI:
    ...
    async def get_filepath(
        ...
        if audio:
            try:
                extension = audio.file_name.rsplit(".", 1)[-1].lower()
                file_name = (
                    audio.file_unique_id
                    + "."
                    + (
                        (extension if extension in allowed_extensions else "mp3")
        ...
        if video:
            try:
                extension = video.file_name.rsplit(".", 1)[-1].lower()
                file_name = video.file_unique_id + "." + (
                    extension if extension in allowed_extensions else "mp4"
                )
            except:
