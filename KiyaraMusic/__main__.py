...
import config
from KiyaraMusic import LOGGER, app, userbot
from KiyaraMusic.core.call import Nand
from KiyaraMusic.misc import sudo
from KiyaraMusic.plugins import ALL_MODULES
from KiyaraMusic.utils.database import get_banned_users, get_gbanned
...
async def setup_bot_commands():
    try:
        ...
        LOGGER("KiyaraMusic").info("Bot commands set successfully!")
        
    except Exception as e:
        LOGGER("KiyaraMusic").error(f"Failed to set bot commands: {str(e)}")
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("KiyaraMusic.plugins" + all_module)

    LOGGER("KiyaraMusic.plugins").info("Successfully Imported Modules...")
    ...
    LOGGER("KiyaraMusic").info(
        "Kiyara Music Started Successfully.\n\nDon't forget to visit @KiyaraBots"
    )
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("KiyaraMusic").info("Stopping Kiyara Music Bot...🥺")
